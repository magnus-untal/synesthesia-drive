...
public class PhyllotaxisXZ : MonoBehaviour
{
    ...
    void SetLerpPositions(){
        ...
        _endPosition = new Vector3(_phyllotaxisPosition.x, 0, _phyllotaxisPosition.y);
    }
    ...
    void Update(){
        ...
        if(!_useLerping){
            _phyllotaxisPosition = CalculatePhyllotaxis(_degree, _currentScale, _number);
            transform.localPosition = new Vector3(_phyllotaxisPosition.x, 0, _phyllotaxisPosition.y);
            _number+= _stepSize;
            _currentIteration++;
        }
    }
