...
public class LevelLayoutGenerator : MonoBehaviour
{
    ...
    void PickAndSpawnChunk()
    {
        ...
        GameObject objectFromChunk = chunkToSpawn.levelChunks[0];
        previousChunk = chunkToSpawn;
        Instantiate(objectFromChunk, spawnPosition + spawnOrigin, Quaternion.identity);

        Debug.Log("Spawn position x:" + (spawnPosition + spawnOrigin).x + " y:" + (spawnPosition + spawnOrigin).y + " z:" + (spawnPosition + spawnOrigin).z);
    }

    public void UpdateSpawnOrigin(Vector3 originDelta)
    {
        spawnOrigin =  new Vector3(0,0,originDelta.z- 15);
    }
}
